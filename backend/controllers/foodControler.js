...
const searchFood = async(req, res) =>{
  try{
    ...
    if (!keyword) {
      return res.json({ success: false, message: "Enter search text" });
    }

 const searchRes = await foodModel.find({name :{$regex: keyword , $options: "i"}})
 
 if(searchRes.length === 0)
 {
  return res.json({success: false});
 }
 res.json({success: true , data : searchRes });
  }
  catch(error){
